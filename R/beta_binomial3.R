...
#'
#' @export
beta_binomial3_lpmf <- function(y, mu, phi, trials) {
  a <- mu * phi
  b <- (1 - mu) * phi
...
#'
#' @export
beta_binomial3_rng <- function(mu, phi, trials) {
  a <- mu * phi
  b <- (1 - mu) * phi
...
#'
#' @export
log_lik_beta_binomial3 <- function(i, prep) {
  mu <- prep$dpars$mu[, i]
  phi <- prep$dpars$phi
  trials <- prep$data$trials[i]
  y <- prep$data$Y[i]
  beta_binomial3_lpmf(y, mu, phi, trials)
}

#' posterior_predict_beta_binomial3
#'
...
#'
#' @export
posterior_predict_beta_binomial3 <- function(i, prep, ...) {
  mu <- prep$dpars$mu[, i]
  phi <- prep$dpars$phi
  trials <- prep$data$trials[i]
  beta_binomial3_rng(mu, phi, trials)
}

#' posterior_epred_beta_binomial3
#'
...
#'
#' @export
posterior_epred_beta_binomial3 <- function(prep) {
  mu <- prep$dpars$mu
